 ...
 * [overrides the base hop function so things can hop on the z axis]
 */

public class ZAxisHopScript : BaseHopScript
{
    /// <summary>
    ...
    public override void HandleHop()
    {
        if (_isHandlingJump && !_frozen)
        {
            ...
            {
                movePos.z = GetHeight(distanceU, -.5f);
            }
            else
            {
                movePos.z = GetHeight(distanceU, -1.5f);
            }
    ...
    protected float GetHeight(float distPercent, float jumpHeight)
    {
        if (distPercent <= .5f)
        {
            ...
            return (1 - jumpU) * _startPos.z + jumpU * (_startPos.z + jumpHeight);
        }
        else
        {
            float fallU = (distPercent - .5f) / .5f;
            fallU = Mathf.Pow(fallU, 2.5f);
            return (1 - fallU) * (_startPos.z + jumpHeight) + fallU * _endPos.z;
        }
    }
        ...
        if (_model != null)
        {
            switch (_currentDirection)
            {
                ...
                case DirectionEnum.UpLeft:
                    _model.transform.eulerAngles = new Vector3(0, 0, 180);
                    break;
                case DirectionEnum.DownLeft:
                    _model.transform.eulerAngles = new Vector3(0, 0, 270);
                    break;
            }
        }
    }
